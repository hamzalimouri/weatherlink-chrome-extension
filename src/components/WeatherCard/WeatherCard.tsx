...
const WeatherContainer: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  return (
    <Box mx={'4px'} my={'16px'}>
...
const WeatherCard: React.FC<{ city: string }> = ({ city }) => {
  ...
  if (state === 'loading' || state === 'error') {
    return <WeatherContainer>
	  ...
	  <Typography variant='body1' component='div' color={state === 'loading' ? 'blue' : 'red'}>
		{state === 'loading' ? 'Loading...' : 'Error: Could not load weather data'}
	  </Typography>
	</WeatherContainer>
  }
